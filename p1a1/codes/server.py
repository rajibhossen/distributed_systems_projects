...
SERVER_DIR = "/home/rajib/Courses/Distributed_Systems_5306/project1/server_dir/"

def upload_file(filename, connection, address):
    print "["+str(address)+"] File Upload Operation.."
...
def rename_operation(old_name, new_name, connection, address):
    print "["+str(address)+"] File Rename Operation.."
    
    old_name = SERVER_DIR + old_name
    new_name = SERVER_DIR + new_name

    if not os.path.isfile(old_name):
        print "["+str(address)+"] File not found"
        connection.send("File not found")
    else:
        os.rename(old_name, new_name)
        print "["+str(address)+"] File renamed successfully: " + old_name + " ->" + new_name
        connection.send("rename successfull")
    return

def delete_operation(filename, connection, address):
    print "["+str(address)+"] File Deletion operation.."
    filepath = SERVER_DIR + filename
    if os.path.isfile(filepath):
        os.remove(filepath)
        print "["+str(address)+"] File deleted successfully"
...
def download_operation(filename, connection, address):
    ...
    if os.path.isfile(filename):
        ...
        with open(filename, 'rb') as f_send:
            print "["+ str(address) +"]"+ "Sending Data.."
            data = f_send.read()
            connection.send(data)
            print "["+ str(address) +"]"+ "Sent"

        print "["+str(address)+"] File sent succesfully"
    else:
        print "["+str(address)+"] File not exists"
