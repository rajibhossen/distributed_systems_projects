...
def client_option():
    ...
    while True:
        connection = get_socket()
        menu = raw_input("Enter your choices: upload, download, rename, delete, exit: ")
        if menu == "upload":
            filename = raw_input("Enter File name to upload: ")
            upload_file(filename, connection)
        elif menu == "rename":
            old_name = raw_input("Old file name: ")
            new_name = raw_input("New file name: ")
            rename_file(old_name, new_name, connection)
        elif menu == "delete":
            filename = raw_input("File name to delete: ")
            delete_file(filename, connection)
        elif menu == "download":
            filename = raw_input("File name to download: ")
